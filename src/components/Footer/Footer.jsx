...
import toast, { Toaster } from 'react-hot-toast'; // <--- Importación

// --- Styled Components --- (Se mantienen tus estilos originales)
const Container = styled.div`
  ...
  @media (max-width: 840px) { width: 90%; }
  @media (max-width: 650px) { flex-direction: column; gap: 3rem; }
  @media (min-width: 1920px) { padding: 3rem 0; }
  @media (min-width: 2560px) { padding: 4rem 0; }
`;
...
const Profile = styled.div`
  ...
  .profiles .icons {
    ...
    span {
      background-color: #000; width: 3rem; height: 3rem; border-radius: 50%;
      display: flex; align-items: center; justify-content: center;
      margin-right: 0.5rem; transition: background-color 0.3s ease;
      a {
        display: flex; align-items: center; justify-content: center;
        width: 100%; height: 100%; border-radius: 50%; color: #fff;
        text-decoration: none;
        svg { font-size: 1.6rem; transition: transform 0.3s ease; }
        &:hover svg { transform: scale(1.2); }
      }
      &:hover { background-color: #01be96; }
    }
  }
`;
...
const ArrowUp = styled.div`
  ...
  background-color: #01be96; display: flex; align-items: center; justify-content: center;
  cursor: pointer; font-size: 1.3rem; font-weight: 700; border-radius: 50%;
  box-shadow: 0 0 8px #01be9551; z-index: 999;
  &:hover { box-shadow: 0 0 12px #01be9570; }
  @media (max-width: 650px) { bottom: 25px; right: 15px; left: auto; transform: none; }
`;
...
const Form = styled.div`
  ...
  padding-bottom: 4rem;
  form {
    background-color: #191923; padding: 0.8rem; border-radius: 5px;
    .name, .email, .message {
      display: flex; border: 1px solid gray; margin-bottom: 0.5rem;
      input, textarea {
        width: 100%; border: none; outline: none; color: #fff;
        background-color: transparent; padding: 1rem 0.5rem;
      }
      span { background-color: #3e3e3e; width: 3rem; display: flex; align-items: center; justify-content: center; }
...
const Footer = () => {
  ...
  const sendEmail = (e) => {
    ...
    // Configuración del Toast Avanzado
    toast.promise(emailPromise, {
      loading: t('footer.sending') || 'Enviando...',
      success: () => {
        e.target.reset();
        return t('footer.successMsg');
      },
      error: (err) => {
        console.error('EmailJS error:', err);
        return t('footer.errorMsg');
      },
    }, {
      ...
      success: {
        duration: 5000,
        iconTheme: { primary: '#01be96', secondary: '#fff' },
      },
    });
  };
